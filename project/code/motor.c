...
void Motor_Init(void) 
{
  ...
  gpio_init(C4, GPO, GPIO_HIGH, GPO_PUSH_PULL);
}
...
void Motor_SetSpeedright(int16_t Speed)
{
		...
		if (Speed >= 0)							//如果设置正转的速度值
	{
		gpio_set_level(C2, GPIO_HIGH);	//c2置高电平
		gpio_set_level(C3, GPIO_LOW);	//c3置低电平，设置方向为正转
		pwm_set_duty(TIM5_PWM_CH4_A3, Speed); 				//PWM设置为速度值
	}
	else									//否则，即设置反转的速度值
	{
		gpio_set_level(C3, GPIO_HIGH);	//c3置高电平
		gpio_set_level(C2, GPIO_LOW);		//c2置低电平，设置方向为反转
